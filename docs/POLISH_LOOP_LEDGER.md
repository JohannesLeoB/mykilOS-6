...
| S8 | Kontakte-Widget im Projektdetail reparieren | done | (dieser Commit) | 344 | Ursache: People-API `searchContacts` liefert beim kalten Index **deterministisch leer** (Warmup-Pflicht). Fix: stiller Warmup (leere Query) + Retry-once; Query-Normalisierung (Unterstrich→Space, „Fuckner_Huetter"). +2 Tests (buildWarmupURL, normalizedQuery) |
